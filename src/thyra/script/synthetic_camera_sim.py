#!/usr/bin/env python3
"""
synthetic_camera_sim.py
-----------------------
Original synthetic camera with grid visualization.
Now supports gimbal pitch commands via /gimbal/cmd_pitch.
...
class SyntheticCameraNode(Node):
    def __init__(self):
        ...
        # Fixed Camera Mounting (45 degrees forward-down) — default
        ...
        self.get_logger().info("Synthetic Camera Started (gimbal support enabled)")
    ...
    def timer_cb(self):
        ...
        # Use gimbal pitch if available, otherwise default mount pitch
        cam_pitch = self.gimbal_pitch_override if self.gimbal_pitch_override is not None else self.mount_pitch
        R_mount = self.get_rotation_matrix(0, cam_pitch, 0)
