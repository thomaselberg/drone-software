...
aruco_detector.py
-----------------
ArUco detection node (Python). The C++ implementation in
src/aruco_detector.cpp is the production version; this script is kept
for quick iteration in sim.
...
Pipeline:
  ...
  5. Publish detection result as a Vector3Stamped:
       x = ground-projected North error in meters
       y = ground-projected East  error in meters
       z = lock flag (1.0 = locked, 0.0 = no detection)
     header.frame_id = relative yaw (degrees, marker − drone) as string.

ArUco forward direction convention:
  The marker's "top" edge (TL→TR, corners 0→1) points forward (in the
  velocity direction). Forward vector = midpoint(BL,BR) → midpoint(TL,TR),
  both projected to the ground plane.
...
Publishers:
  /asr/aruco/pixel_error            (geometry_msgs/Vector3Stamped)
...
class ArucoDetector(Node):
    ...
    def __init__(self):
        ...
        self.pub_error = self.create_publisher(
            Vector3Stamped, '/asr/aruco/pixel_error', 1)
